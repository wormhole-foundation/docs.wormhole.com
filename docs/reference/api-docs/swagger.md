...
Global transactions are a logical association of two transactions that are related to each other by a unique VAA ID.
...
Total messages are the number of VAAs emitted since the creation of the network (includes Pyth messages).
...
The volume is calculated using the notional price of the symbol on the day the VAA was emitted.
...
the price that has the most occurrences in all the nodes for an originChainId and originAddress is returned.
