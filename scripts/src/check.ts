...
function checkWhSdk() {
  for (const [network, chainContracts] of Object.entries(whcontracts)) {
    ...
    for (const [chain, contracts] of Object.entries(chainContracts)) {
      ...
      const newContracts =
        newconf[net].chains[idToChainName.get(id)!]?.contracts;
      ...
      if (contracts.core !== newContracts.coreBridge)
        console.log(
          `Core bridge mismatch for ${id} ${net} ${chain}: ${contracts.core} vs ${newContracts.coreBridge}`
        );

      if (contracts.token_bridge !== newContracts.tokenBridge)
        console.log(
          `Token bridge mismatch for ${id} ${net} ${chain}: ${contracts.token_bridge} vs ${newContracts.tokenBridge}`
        );

      if (contracts.nft_bridge !== newContracts.nftBridge)
        console.log(
          `NFT bridge mismatch for ${id} ${net} ${chain}: ${contracts.nft_bridge} vs ${newContracts.nftBridge}`
        );
    }
  }

  for (const [network, chainContracts] of Object.entries(
    relayer.RELAYER_CONTRACTS
  )) {
    const net = mapnet(network);
    for (const [chain, contracts] of Object.entries(chainContracts)) {
      // @ts-ignore
      ...
      if (contracts.wormholeRelayerAddress !== newContracts.relayer)
        console.log(
          `Relayer mismatch for ${id} ${net} ${chain}: ${contracts.wormholeRelayerAddress} vs ${newContracts.relayer}`
        );
    }
  }
}
